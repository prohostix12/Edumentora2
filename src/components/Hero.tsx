...
export default function Hero() {
  ...
  return (
    <section className="relative w-full h-[100dvh] flex overflow-hidden font-[Poppins]">
      ...
      <aside className="hidden md:flex flex-col w-[240px] lg:w-[280px] h-full shrink-0 bg-gradient-to-b from-[#1B4B43] to-[#123832] relative overflow-hidden">
        <div
      ...
      <div className="relative flex-1 h-full flex flex-col pt-32 lg:pt-[100px] pb-0 bg-[#FBF3E7] overflow-hidden">
      ...
      <div className="md:hidden relative z-10 px-4 mb-4 shrink-0">
        <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
          {ACHIEVEMENTS.map((item) => (
            <div
              key={item.label}
              className="shrink-0 bg-white rounded-xl border border-[#1B4B43]/10 shadow-sm px-4 py-2.5 min-w-[132px]"
            >
              <div className="text-[#1B4B43] text-lg font-extrabold leading-tight">{item.value}</div>
        ...
        <div className="flex flex-col md:flex-row flex-1 min-h-0 gap-8 lg:gap-10 items-stretch">
          ...
          <div className="flex flex-col items-start justify-center text-left h-full md:flex-[1.3]">
            ...
            >
              ...
              <span className="font-bold text-[#1B4B43] text-sm">UGC &amp; AICTE Recognized</span>
            </motion.div>

            <motion.p
              {...riseIn(0.1)}
              className="text-[#1B4B43] font-bold text-base md:text-lg mb-3"
            >
              Ready to finish the degree you already started?
            </motion.p>

            <h1 className="text-4xl sm:text-5xl xl:text-6xl font-extrabold text-[#1B4B43] leading-[1.05] tracking-tight mb-6 h-[190px] sm:h-[170px] md:h-[200px] xl:h-[220px] flex flex-col justify-end">
              <div>
            ...
            >
              {FEATURES.map(({ icon: Icon, label }) => (
                <span key={label} className="flex items-center gap-2 text-[#1B4B43] font-bold text-sm">
          ...
          >
            ...
            <svg
              className="hero-doodle hidden lg:block absolute -top-6 left-6 w-14 h-10 text-[#1B4B43]/70 pointer-events-none"
            ...
            <svg
              className="hero-doodle hidden lg:block absolute top-10 -right-4 w-10 h-10 text-[#1B4B43] pointer-events-none"
            ...
            <div className="absolute inset-x-8 inset-y-6 rounded-[3rem] bg-gradient-to-br from-[#1B4B43]/25 via-[#C9A66B]/15 to-transparent blur-2xl -z-10" />

            <div id="eligibility-form" className="relative w-full max-w-[360px] rounded-[2.5rem] border-4 border-[#1B4B43]/70 shadow-[0_25px_60px_-20px_rgba(23,42,83,0.35)] bg-white p-6 scroll-mt-32">
              <div className="flex items-center gap-2 mb-3 rounded-full bg-blue-50 w-fit px-3 py-1.5">
                <ClipboardCheck className="w-4 h-4 text-[#1B4B43]" />
                <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#1B4B43]">Fast Assessment</span>
              </div>
              <h3 className="text-xl font-bold text-[#1B4B43] mb-1">Check Your Eligibility Now</h3>
      ...
      <div className="relative z-10 w-full shrink-0 bg-[#1B4B43] py-3 overflow-hidden">
        <div
